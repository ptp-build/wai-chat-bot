...
export default class ProtoController extends WaiOpenAPIRoute {
	...
	async dispatch(request: Request) {
		try {
			...
			// @ts-ignore
			return WaiOpenAPIRoute.responseError(
				ENV.IS_PROD ? 'System Error' : e.stack.split('\n')
			);
		}
	}
		...
		if (userStoreDataStr) {
			...
			if (userStoreData?.chatIds) {
				userStoreData.chatIds.forEach(id => {
					...
					if (
						!userStoreDataRes.chatIds.includes(id) &&
						!userStoreDataRes.chatIdsDeleted.includes(id)
					) {
